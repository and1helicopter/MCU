...
int  i;
  ...
  if (RxBuff[0]  != 0x3A) {return 1;}					
  if (RxBuff[57] != 0x0D) {return 1;}					
  if (RxBuff[58] != 0x0A) {return 1;}					
...
int  TermoRequest(void) {
  if (TermodatStatus)	{
	  switch (TermodatStep) {
	    case 0x00: {SetParam(0x01B0,0x0001,12-TermodatStatus,TxBufferTD);} break;
	    case 0x01: {SetParam(0x01B1,0x0000,12-TermodatStatus,TxBufferTD);} break;
	    case 0x02: {SetParam(0x01B5,  1000,12-TermodatStatus,TxBufferTD);} break;
	  }
	  TermodatBUSY = 1;
		return 17;
	}
...
int TxCount;
  ...
  if (TermodatInit) {TermodatInit = 0; TermodatStep = 0; TermodatStatus = 12;}	
...
short T;
	...
	if (Status & (((uint16_t)0x0001)|((uint16_t)0x0008)|((uint16_t)0x0020))) {
  ...
  for (i = 0;  i < 12;  i++) {
		T = (ASCIIToChar(RxBuffer[(i<<2)+ 7])<<12)|	
				...
				Metro . WorkingEvent . EventData .TD[i] = T;
  }
  TermodatError = 0;
  TermodatBUSY = 0;
}
