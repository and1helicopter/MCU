...
static int Buffer = 0;
	...
	if (FiltCount) {
			if (CurrCh > 15) {Value = AP_REG(Metro . StdCmnd . StdCmnd .IntADCData[CurrCh - 16] << 3, 656, &Buffer);}
				else 	{Value = AP_REG(Metro . StdCmnd . StdCmnd .ExtADCData[CurrCh], 656, &Buffer);}  
			if (--FiltCount == 0)	{
				Metro . SavedParam . SavedParam .ADCOffs[CurrCh] = Value;
				if (ChsToZero) {
					FiltCount = 1000; 
					Buffer = 0;
