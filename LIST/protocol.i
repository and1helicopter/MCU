...
uint32_t Addr;	
  ...
  if ((i <= 0) || (i > ((128 - 5) >> 1) )) return ModbusErr(0x02, RxBuff, TxBuff);
...
union UsartPrms_union Prms;
	...
	if (Status & (((uint16_t)0x0001)|((uint16_t)0x0008)|((uint16_t)0x0020))) return 0;				 
	if (RxCount < 4) return 0;																							 
	if (CalcCRC(RxCount, RxBuffer)) return 0;																 
																																					 
	Prms.AllParams = GetSavedUartParams(PortNum);
	if (Prms.Params.Addr1 != RxBuffer[0]) {return 0;}

	switch (RxBuffer[1]) {
	  case 0x03: return Modbus03(RxCount, RxBuffer, TxBuffer, (unsigned short *)&Metro, sizeof(Metro)); break;
		case 0x04: return Modbus04ExtMem(RxCount, RxBuffer, TxBuffer, (uint16_t *)&ExtMem . w16[0], sizeof(ExtMem . w16)); break;
		case 0x06: return Modbus06(RxCount, RxBuffer, TxBuffer, (unsigned short *)&Metro, sizeof(Metro)); break;
		case 0x08: return Modbus08(RxCount, RxBuffer, TxBuffer, (unsigned short *)&Metro, sizeof(Metro)); break;
		case 0x10: return Modbus16(RxCount, RxBuffer, TxBuffer, (unsigned short *)&Metro, sizeof(Metro)); break;
...
int i;
	...
	for (i = 0; i < 24; i++) {Metro . WorkingEvent . EventData .T[i] = RxBuffer[i];}
...
int i;
	...
	if (PortNum == 1) {
		if (RxBuffer[0] == 0x01) {SendDataPtr = Metro . WorkingEvent . EventData .IGBTStat3;} else {SendDataPtr = Metro . WorkingEvent . EventData .IGBTStat1;} 
	} else if (PortNum == 2) {
		if (RxBuffer[0] == 0x01) {SendDataPtr = Metro . WorkingEvent . EventData .IGBTStat4;} else {SendDataPtr = Metro . WorkingEvent . EventData .IGBTStat2;} 
	} else return 0;
