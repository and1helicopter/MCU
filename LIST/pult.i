...
__align(4) unsigned char TableRusLCD[] = {
									...
									0x0E,0x61,0x62,0x63,0x64,0x65,0x66,0x67,0x68,0x69,0x6A,0x6B,0x6C,0x6D,0x6E,0x6F,	 
									 
									0x70,0x71,0x72,0x73,0x74,0x75,0x76,0x77,0x78,0x79,0x7A,0xD9,0x08,0xDA,0x0D,0x0E,	 
									 
									0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,	 
									0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,	 
									0x20,0x20,0x20,0x20,0x20,0x20,0x09,0x0E,0xA2,0x20,0x20,0x0B,0x20,0x20,0x20,0x20,	 
									 																			
									0xEF,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0xB5,0x09,0x20,0x0C,0x20,0x20,0x20,0x20,	 
...
void InitPult(void) {
	...
	if (((SPI_TypeDef *) (((uint32_t)0x40000000) + 0x3C00)) == ((SPI_TypeDef *) (((uint32_t)0x40000000) + 0x3C00))) {																								
		if (((RCC_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x3800))->APB1ENR & ((uint32_t)0x00008000)) return;											 
		((RCC_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x3800))->APB1ENR |= ((uint32_t)0x00008000);																	 
	} else return;
	
	((SPI_TypeDef *) (((uint32_t)0x40000000) + 0x3C00))->CR1 = 	((uint16_t)0x0040) |				 
									((uint16_t)0x0200) | 			 
									((uint16_t)0x0100) | 			 	
									((uint16_t)0x0004)| 			 
...
int WorkPultCount;
  ...
  if (WorkPultCount > 0)  WorkPultKeyb |= ((SPI_TypeDef *) (((uint32_t)0x40000000) + 0x3C00))->DR;
  if (WorkPultCount < 36)	((SPI_TypeDef *) (((uint32_t)0x40000000) + 0x3C00))->DR = WorkPultBuff[WorkPultCount];
...
int i;	
	if (PultCount < 37) return 1;																											 
	
  for (i = 8; i >= 0; i--) {((int*)WorkPultBuff)[i] = ((int*)SendPultBuff)[i];}			 	

  WorkPultBuff[0]  = (Leds&0x3F)|0x40;																							 							
	SendPultBuff[35] = 0;																															 
...
while (1) {
	...
	ClearScreen();
	lcdoutDT(Metro . WorkingEvent . EventData .RTC_Date,Metro . WorkingEvent . EventData .RTC_Time,Metro . WorkingEvent . EventData .RTC_SSC);
	while (PultRefresh());
	}  
}	 	
...
int i;
for (i = 15; i >= 0; i--) { SendPultBuff[(pos++&0x1F)+3] = 0x30 + ((num >> i) & 0x0001);} 
} 
...
int pos = 3;
int i;
char DayWeek[16] = {' ',' ',0xA8,'H','B','T','C','P',0xAB,'T',0xA8,'T','C',0xA0,'B','C'};		 
	
		SS = ((SS * 5) >> 7);
		_snprintf(&SendPultBuff[3],33," : %2.2x/%2.2x/%4.4x   B: %2.2x:%2.2x:%2.2x.%1.1u   ",																	D&0x3F,(D>>8)&0x1F,((D>>16)&0xFF)|0x2000,(T>>16)&0x3F,(T>>8)&0x7F,T&0x7F,SS);
...
char DayWeek[16] = {' ',' ',0xA8,'H','B','T','C','P',0xAB,'T',0xA8,'T','C',0xA0,'B','C'};		 
		...
		SS = ((SS * 1000) >> 8);
		_snprintf(&SendPultBuff[3],33," : %2.2x/%2.2x/%4.4x   B: %2.2x:%2.2x:%2.2x.%3.3u   ",																	D&0x3F,(D>>8)&0x1F,((D>>16)&0xFF)|0x2000,(T>>16)&0x3F,(T>>8)&0x7F,T&0x7F,SS);
...
int 	ChangeParam(int Count, char *Header, char *Strs) {
	...
	while (1) {
		...
		if ((ChangePrms.Flags[ChangePrms.Param] & TiFlag) && (ChangePrms.Value[ChangePrms.Param] == 0)) {lcdout(22,7," ÎÒÊË. ");} else	
