...
int main() {
int i;	
	for (i=0; i<240; i++) ((NVIC_Type *) ((0xE000E000UL) + 0x0100UL) )->IP[i] = 0xF0;										 
	SystemCoreClockUpdate();																		 
	SysTick_Config(SystemCoreClock / 8 / 1000, SysTick_Prty);		 
	...
	Metro . StdCmnd . StdCmnd .ParamsLoaded = LoadParamsProc();
	InitParams(Metro . StdCmnd . StdCmnd .ParamsLoaded);
	 
	InitOscil(&Metro . SavedParam . SavedParam .Oscil, &Metro .OscilCmnd, (uint16_t*)&Metro, (uint16_t*)&ExtMem);
	...
	InitUsart(3, Baud9600 | NoParity | OneStopBit, RecieveTermodat);

	
  InitUsart(3, Baud115200 | OddParity | OneStopBit, ModbusStdProc);
	InitUsart(4, Baud115200 | OddParity | OneStopBit, ModbusStdProc);	

	InitUsart(5, Baud115200 | OddParity | OneStopBit, RecieveSlaveData);	
	InitUsart(1, Baud115200 | OddParity | OneStopBit, RecieveDriverData);		
	InitUsart(2, Baud115200 | OddParity | OneStopBit, RecieveDriverData);	
	...
	Metro . WorkingEvent . EventData .EventCode = 0x0053;
...
static int LedCount = 0;
	...
	if (Metro . StdCmnd . StdCmnd .SetDate && Metro . StdCmnd . StdCmnd .SetTime) {PackRTC(Metro . StdCmnd . StdCmnd .SetTime,Metro . StdCmnd . StdCmnd .SetDate); Metro . StdCmnd . StdCmnd .SetTime = Metro . StdCmnd . StdCmnd .SetDate = 0;}

 
	SetZeroChs(&Metro . StdCmnd . StdCmnd .SetZeroCmd);
