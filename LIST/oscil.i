...
uint16_t *WorkPtr;  
  ...
  if (ArrayIndex > (((1048576/2) >> 1) - 1)) return;
  if ((ArrayIndex + (OscilSampleSize >> 1)) > ((1048576/2) >> 1)) return;
  ...
  for (i = 0; i < WorkOscilConfig.OscilChNum; i++)
  {
    ...
    {
      *((uint64_t *)(&ptrExtMem[ArrayIndex])) = *((uint64_t *)(WorkPtr));     
      ArrayIndex += 4;
    }
    else if ((WorkOscilConfig.OscilTypeData[i] & 0x0300) == 0x0200)           
    {
      *((uint32_t *)(&ptrExtMem[ArrayIndex])) = *((uint32_t *)(WorkPtr));     
      ArrayIndex += 2;      
    } 
    else if ((WorkOscilConfig.OscilTypeData[i] & 0x0300) == 0x0100)
    {
      ptrExtMem[ArrayIndex++] = *(WorkPtr);                                   
    } 
  }
}
...
void TimeStamp(int32_t OscilNum) {
  ...
  ptrOscilCmnd->OscilDateTime[(OscilNum * 2) + 0] = Metro . WorkingEvent . EventData .RTC_Date & 0x00FF1F3F;    
  ptrOscilCmnd->OscilDateTime[(OscilNum * 2) + 1] = Metro . WorkingEvent . EventData .RTC_Time;
...
int32_t WorkOscilNum;
  ...
  switch (ptrOscilCmnd->OscilStatus[WorkOscilNum]) 
  {
    ...
    case 0x02:  {
      ...
      if (Metro . StdCmnd . StdCmnd .CycleCounter) break;  
...
int32_t i;
  ...
  if ((OscilConfig->OscilSize * OscilConfig->OscilQuantity) > (1048576/2)) {return 8;}
...
static  int16_t OscilCfgLoadCount = 0;
  ...
  {	
    ...
    if (OscilCfgLoadCount == ptrOscilCmnd->OscilNewConfig[1])
    {
      for (i = 0 ; i < 32; i++)
      {	
        ((uint16_t *)(&NewOscilConfig))[32 * OscilCfgLoadCount + i] = ptrOscilCmnd->OscilNewConfig[i + 2]; 
      }	      
      OscilCfgLoadCount++;              
    } else return;
...
void InitOscil(OscilConfig_type *SavedOscilConfig, OscilCmnd_type *OscilCmnd, uint16_t *DataBlock, uint16_t *Memory) {
    ...
    ptrDataBlock = DataBlock; 
		...
		ptrOscilCmnd->OscilMemorySize = (1048576/2);								
		ptrOscilCmnd->OscilSampleRate = 16000;							  
